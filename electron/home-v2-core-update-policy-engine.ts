...
export function createHomeV2CoreUpdatePolicyEngine(dependencies: Dependencies) {
  ...
  return {
    ...
    async runPass() {
      ...
      if (!checkCore && !checkJava) return activity
      ...
      try {
        ...
        const javaCandidate = java?.source === 'managed' &&
          java.managedUpgradeAvailable === true &&
          typeof java.updateAvailableVersion === 'string'
        ...
        if (javaCandidate && settings.javaUpdatePolicy === 'install') {
          ...
          activity = {
            ...
            generation: settings.generation,
            issue: null,
            java: { state: javaState, version: java.updateAvailableVersion as string },
          }
          try {
          ...
          try {
            ...
            if (runtime !== 'stopped') {
              ...
              activity = {
                ...
                java: {
                  state: javaState,
                  version: javaCandidate ? java.updateAvailableVersion as string : null,
                },
              }
        ...
        activity = {
          checkedAt: new Date().toISOString(),
          core: { channel, state: checkCore ? coreState : 'idle', version: coreCandidate ? release.tagName : null },
          generation: settings.generation,
          issue,
          java: {
