...
{
  const encrypted = encryptQortalPublicKeyEnvelopeForTest({
    data64: PLAINTEXT,
...
// --- Round trip ------------------------------------------------------------
{
  ...
  const asBob = decryptQortalPublicKeyEnvelope({ encryptedBase64: encrypted, readerPrivateKey: bob.secretKey })
  ...
  const asAlice = decryptQortalPublicKeyEnvelope({
  ...
  assert.throws(
    () => decryptQortalPublicKeyEnvelope({ encryptedBase64: encrypted, readerPrivateKey: carol.secretKey }),
...
// Encrypting to nobody but yourself is legitimate: app-private storage.
{
  ...
  assert.equal(
    decryptQortalPublicKeyEnvelope({ encryptedBase64: encrypted, readerPrivateKey: alice.secretKey }).data64,
    PLAINTEXT,
  )
}
...
// what a reader uses to find the key block, so a duplicate would misalign it.
{
  ...
  assert.equal(
    decryptQortalPublicKeyEnvelope({ encryptedBase64: encrypted, readerPrivateKey: bob.secretKey }).data64,
    PLAINTEXT,
  )
}
...
for (const [input, pattern] of [
  ...
  assert.throws(
    () => decryptQortalPublicKeyEnvelope({ encryptedBase64: input, readerPrivateKey: alice.secretKey }),
    pattern,
  )
}
// A truncated envelope must refuse rather than read past its own data.
{
  ...
  assert.throws(
    () => decryptQortalPublicKeyEnvelope({ encryptedBase64: truncated, readerPrivateKey: bob.secretKey }),
...
// it. This vector can only be opened with the raw X25519 secret.
{
  const hubEnvelope =
    ...
    'fC7mKIo7hgIAAAA='
  const asBob = decryptQortalPublicKeyEnvelope({
  ...
  assert.equal(
    decryptQortalPublicKeyEnvelope({ encryptedBase64: hubEnvelope, readerPrivateKey: alice.secretKey }).data64,
    PLAINTEXT,
    'and so does the sender, as Hub intends',
  )
  assert.throws(
    () => decryptQortalPublicKeyEnvelope({ encryptedBase64: hubEnvelope, readerPrivateKey: carol.secretKey }),
    /not encrypted for the selected account/,
  )
}
...
// opened envelope as proof of the original recipient set.
{
  ...
  const survivor = decryptQortalPublicKeyEnvelope({
