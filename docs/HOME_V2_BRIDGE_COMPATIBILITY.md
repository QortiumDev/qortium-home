...
| `GET_QDN_RESOURCE_METADATA`, `GET_QDN_RESOURCE_PROPERTIES`, `GET_QDN_RESOURCE_STATUS`, `GET_QDN_RESOURCE_URL` | both | Bare JSON or resource URL; FILE/FILES URLs address raw bytes, other services retain render routes | Source protocol selects chain; Android FILE/FILES URLs use the authorized HTTPS stream proxy; see [resource URL contract](QDN_RESOURCE_VIEWER.md#obtain-a-resource-url) | yes | yes |
...
| `UNLOCK_SELECTED_ACCOUNT` | both | `{ address, avatarContract, avatarUrl, isUnlocked: true, name }` | Already-unlocked account: answered with no prompt, gated exactly like `GET_SELECTED_ACCOUNT` (selected account present, live resource unchanged), and the unlock state is re-asserted after the profile read (2026-09-20). Locked account: single-request prompt, never a session or durable grant; opens Home's own password dialog naming the requesting app, asserts the unlock actually completed, and rechecks app/tab/account/route before returning. Advertised on **both protocols and both platforms**: unlocking is a Home-account operation, not a chain one — the same wallet, dialog and key whichever global asked — and the legacy wallet app only knows the `qortalRequest` global. The route recheck (desktop and Android alike) binds to the REQUEST's own network, so a `qortalRequest` unlock is checked against the Qortal route. No app receives passwords, seeds, derived key bytes, or private keys | yes | yes |
| `OPEN_NEW_TAB` | both | `true` | Only `qdn://`, `qortal://`, or `home://`; Home owns navigation | yes | yes |
| `OPEN_EXTERNAL_LINK` | both | `{ opened: true, url }` or an error | Host-mediated open of one absolute http(s) link in the system browser. Validated before any prompt (http/https only, a host, no credentials, no control characters, ≤ 2,048 chars); then a trusted single-request prompt showing the site and the full link — never a durable grant, never `account.read`, no account needed; on approval Home hands the URL to the OS browser (desktop `shell.openExternal`; Android an explicit `ACTION_VIEW` intent via the `ExternalLink` plugin, never `window.open`, so a link can only leave the app) and fetches nothing itself. Links to loopback hosts (Home's own render surfaces) are refused. One pending link per app, ten overall, requesting tab must be visible — the same on both transports. | yes | yes |
...
| `SEND_QORTAL_GENERAL_CHAT` | `qortalRequest` only | `{ signature, timestamp }` (the inner CHAT's signature, the id Hub and Chat use), or a signed non-retryable unknown-outcome result | Qortal General Chat (group 0) over the MESSAGE-wrapper protocol Hub and the Classic UI read: Home builds and proves the group-0 CHAT, signs it with the account, then wraps it in a fee-less MESSAGE signed by a keypair derived from that signature and broadcasts only the wrapper. `{ message, chatReference? }` — one action for a message, an edit, a delete or a reaction; the Hub envelope in `message` decides which, and a revision's reference is looked up in the unconfirmed MESSAGE pool — every row is verified end to end (inner CHAT signature, derived wrapper sender/recipient, and the outer MESSAGE bytes rebuilt from the row and checked against its signature), decoded pools are cached 15 s per node so repeated lookups cannot drive repeated reads or proof-of-work-scale verification before the prompt (edit/delete must belong to the caller; the target must be an original). Same prompt ("General chat"), grant, rate limit and journal as `SEND_CHAT_MESSAGE`. | yes | yes |
...
| `GET_GROUP`, `GET_ACCOUNT_GROUPS`, `GET_GROUP_JOIN_REQUESTS`, `GET_ACCOUNT_GROUP_JOIN_REQUESTS`, `GET_ADMIN_GROUP_JOIN_REQUESTS`, `GET_ACTIVE_CHATS` | both | Bare Core JSON | No prompt; bounded anonymous public reads; positive-integer `groupId`, address regex, strict booleans, 100-entry page cap where Core has none | yes | yes |
| `GET_GROUP_MEMBERS` | both | Bare Core JSON | No prompt; bounded anonymous public read; positive-integer `groupId` and strict `onlyAdmins`; the ordinary group-read family retains its 100-entry cap, while this action alone accepts an explicit `limit` up to 500 for Minting. An omitted limit is still omitted from the Core query. Explicit `limit: 0` is passed through unchanged to preserve Core's unlimited-page sentinel; Home's existing response-byte bound and route restrictions still apply | yes | yes |
...
| `GET_USER_WALLET` | both for native; foreign on `qdnRequest` | Native: `{ address, assetId: 0, assetName: "Native Asset", native: true }`; foreign: `{ address, coin, publicKey, publickey }`; ARRR: `{ address, coin: "ARRR" }` (no `publicKey`/xpub — none exists) | Native remains permissionless and unlock-free. Foreign supports BTC/LTC/DOGE/DGB/RVN/DASH/NMC/FIRO only, requires unlock plus the session-scoped foreign-wallet disclosure it SHARES with the three balance reads whenever a trusted Qortium Core resolves (bound to that Core's route and label, so one approval covers all four; route-independent with the "Home local wallet" label only when no trusted Core resolves — 2026-09-20), derives public watch material locally without requiring Core or serializing xprv, and is never offered to widgets. **ARRR** (`coin: "ARRR"`) is a separate runtime kind (`electron/arrr-custody.ts`): desktop only, `qdnRequest` only, requires unlock plus the distinct session-scoped `account.arrr-custody.read` disclosure, and the address comes from the admin-trusted Core that receives the wallet entropy — see the wallet-family note | yes | native yes; foreign yes; ARRR no (stated reason: the spending key is derived only in the privileged desktop process) |
| `GET_WALLET_BALANCE`, `GET_USER_WALLET_INFO`, `GET_USER_WALLET_TRANSACTIONS` | `qdnRequest` | Core balance/address-info/transaction payload; ARRR: the atomic decimal balance string (verified/spendable by default, total with `verified: false`) and Core's `SimpleTransaction[]` | Same foreign-wallet session disclosure and exact eight-coin set; xpub-only authenticated POST to reachable local or authenticated custom Qortium Core; redirect refusal, bounded response, Core error 1201 normalization. **ARRR** (`GET_WALLET_BALANCE` and `GET_USER_WALLET_TRANSACTIONS` only — `GET_USER_WALLET_INFO` is xpub-only and refuses it) goes through the custody adapter instead: `text/plain` entropy body to `/crosschain/arrr/walletbalance?verified=true|false` and `/crosschain/arrr/wallettransactions` on the admin-trusted Core, under the `account.arrr-custody.read` grant; one request in flight per Core route; a verified balance Core does not know yet is refused with `ARRR_VERIFIED_BALANCE_UNAVAILABLE` (retryable) rather than answered with the total; Core's 409 `ARRR_WALLET_BUSY` becomes `ARRR_WALLET_BUSY` (retryable) and never another account's data | yes | eight coins yes; ARRR no |
...
| `GET_ARRR_SYNC_STATUS` | `qdnRequest` | Frozen snapshot `{ contract: "qortium-home-arrr-custody-v1", coin: "ARRR", state: DISABLED\|LOADING\|SYNCHRONIZING\|DEGRADED\|READY, ready, message, syncedBlocks, totalBlocks, restartRequired, recoveryState, scannedHeight, tipHeight, totalBalanceAtomic, verifiedBalanceAtomic, observedAt, stale, backendMode, walletIdentityHash, lastError: { code, message } \| null }` | Desktop only, admin-trusted route only (advertised there and nowhere else, never to a widget, withheld on Android with a stated reason). `coin` defaults to ARRR and any other coin is refused. Same `account.arrr-custody.read` session/single-request grant as the three ARRR reads, pinned to the resolved admin route (`nodeRoute` through prompt, grant key and post-approval recheck). POSTs the entropy to `/crosschain/arrr/syncstatus?json=true` and validates the answer field by field: unknown state, negative counter, non-decimal balance or malformed `lastError` → `ARRR_SYNC_STATUS_MALFORMED`; a Core without the structured contract (no `stale`) → `ARRR_SYNC_CONTRACT_UNSUPPORTED`. `ready` is Home's verdict (`state === READY && !stale && !restartRequired`) — a stale or restart-pending READY cannot establish readiness, so apps gate balances on `ready`, not `state`. A newer poll from the same tab replaces one still queued (`ARRR_READ_SUPERSEDED`); the in-flight one is never cancelled | yes | no (stated reason) |
| `STOP_ARRR_SYNC`, `START_ARRR_SYNC` | `qdnRequest` | Request `{ action, coin?: "ARRR" }`; result `{ contract: "qortium-home-arrr-sync-control-v1", coin: "ARRR", operation: "stop"\|"start", scope: "node", completed: true, enabled, observedAt }` | Desktop only, admin-trusted Qortium route, selected unlocked account, never widgets. Every call requires a separate single-request `node.settings.write` approval naming the node, operation, node-wide impact and runtime-only persistence. Serializes with ARRR custody reads and rechecks app/account/unlock/session epoch and exact route/key binding after every await. No key derivation or entropy body; fixed POST `/crosschain/arrr/stop` or `/start`, literal `true` required, then `/crosschain/arrr/status` enabled-state readback. A stop timeout/false is `ARRR_SYNC_CONTROL_FAILED`, not proof of restart-required; post-dispatch transport/context/readback uncertainty is `ARRR_SYNC_CONTROL_UNCERTAIN`. Neither error is automatically retryable. Start means controller enabled, NOT wallet ready; the next approved custody read loads the selected wallet. Stop affects all ARRR accounts/apps on that Core and does not delete data. Settings persistence and Core restart are not changed. Advertised on the available ARRR wallet capability as `syncControlContract: "qortium-home-arrr-sync-control-v1"`. | yes | no (stated reason) |
...
| `GET_CROSSCHAIN_BLOCKCHAINS`, `GET_CROSSCHAIN_SERVER_INFO`, `GET_FOREIGN_FEE`, `GET_SERVER_CONNECTION_HISTORY` | both | Blockchain list with a projected `QORT` row and a `homeWallet` capability per row; bare server array; `{ fee, feePerKb }` or `{ fee }`; bare Core JSON | No prompt; zero-key bounded reads of the node's own `/crosschain` prefix. No wallet seed, key derivation, unlocked account or API key is involved. `coin` is resolved against a strict allowlist (BTC, LTC, DOGE, DGB, RVN, DASH, NMC, FIRO, ARRR) before it can become a URL path segment; `ARRR` is accepted here although Home cannot derive an ARRR wallet, because these reads need no key material and Home 1.x wrongly reused the HD-wallet coin list. `GET_FOREIGN_FEE` normalizes Core's per-kilobyte `feekb` to a per-byte `fee` with ceiling rounding, so a fee never rounds down below what the foreign chain requires. The `ARRR` row's `homeWallet` is its own branch: on desktop with an admin-trusted route and an unlocked account it advertises `read`/`receive` in the new `TRUSTED_CORE_CUSTODY` mode with `send: false`, `sendMode: "NONE"`, `syncStatus: true` and `custodyContract: "qortium-home-arrr-custody-v1"`; otherwise (public/untrusted route, locked account, Android) `read`/`receive` are false with an `unavailableReason`. It is never derived from the bitcoiny send-route probe | yes | yes (ARRR row always unavailable, with the reason) |
...
| Lists, hosted data, files, viewers | `ADD_LIST_ITEMS`, `DELETE_HOSTED_DATA`, `DELETE_LIST_ITEM`, `GET_HOSTED_DATA`, `GET_LIST_ITEMS`, `PLAY_ENCRYPTED_MEDIA`, `SHOW_PDF_READER` (`SAVE_FILE` is implemented in both forms: a QDN `location` maps onto `SAVE_QDN_RESOURCE`, `bytesBase64` onto `SAVE_FILE_BYTES`) (the three list actions have the `qdnRequest` family implemented — see the table above — but these v3 forms stay deferred: every `/lists` route needs the node's administrative key, and Home holds one only for the Qortium Core it runs itself) |
...
| QDN writes | Hub-specific deletion and legacy publication variants beyond the narrow `{ base64, filename }` adapter remain deferred; Home 2's token-only single-resource `PUBLISH_QDN_RESOURCE` — and `PUBLISH_MULTIPLE_QDN_RESOURCES` as a bounded batch of it — use the separate H5B source-token contract. The compatibility adapter supports one top-level pair for a single publish or an all-inline top-level batch of at most ten resources and 25 MiB decoded bytes in aggregate; paths, mixed sources, nested/payload forms, and other encodings remain refused |
...
| Foreign chain and trading | `ADD_FOREIGN_SERVER`, `CANCEL_TRADE_SELL_ORDER`, `CREATE_TRADE_BUY_ORDER`, `CREATE_TRADE_SELL_ORDER`, `REMOVE_FOREIGN_SERVER`, `START_CROSSCHAIN_SERVER`, `UPDATE_FOREIGN_FEE` (the four zero-key `/crosschain` reads, `SET_CURRENT_FOREIGN_SERVER` and, on desktop, `GET_ARRR_SYNC_STATUS` are implemented) |
...
to a trusted Qortium Core. All four foreign reads share ONE
`account.foreign-wallet.read` session grant: when a trusted Qortium Core
resolves, the receive-only `GET_USER_WALLET` consent binds to the same node
route and label as the balance reads (`homeV2ForeignWalletReadConsentBinding`),
so a wallet app opening its receive address and then its balance sees one
prompt, not two (owner decision 2026-09-20). That route is computed ONCE from
the resolved admin node and carried in the disclosure (`nodeRoute`): the
prompt's Node row, the session-grant key and the post-approval recheck
(against a fresh `resolveHomeV2AdminNode`) all use that one value, so the user
can never approve one route while the grant is stored under another. Only when
no trusted Core resolves does the receive-only read fall back to a
route-independent grant labelled "Home local wallet", and the prompt then says
it covers deriving addresses on this device only — the balance reads are
refused outright in that case. The native action's entry in
...
**ARRR is a third runtime kind, not a ninth bitcoiny coin.** Pirate Chain's
shielded wallet has no xpub, and every one of Core's `/crosschain/arrr/*` reads
takes the wallet's 32-byte ENTROPY (Base58, `text/plain` body) — that is the
spending key, and Core keeps a synced copy of the wallet under it. Home 2
therefore models ARRR in `electron/arrr-custody.ts` as an `arrr-custody`
runtime that carries no key material at all (`foreign-wallet-read-contract.ts`
refuses to build an xpub request for it), and serves it on desktop only:

- **Trust.** Custody goes only to an ADMIN-TRUSTED Core — the managed local
  Core with its managed key, or a custom node with the user's own key attached
  (HTTPS anywhere, plain HTTP only to exact loopback) — via the same
  `evaluateHomeV2AdminTrust`/`resolveHomeV2AdminNode` rule the bitcoiny reads
  use. There is no route-independent fallback: the address itself comes from
  the Core that holds the key.
- **Consent.** Its own session-scoped family, `account.arrr-custody.read`
  (write kind `arrr-custody-read`), distinct from `account.foreign-wallet.read`
  even though three of the four actions share their names: neither family's
  grant satisfies the other. Single-request or "for this tab" only, never
  durable. The prompt ("Allow ARRR wallet custody on your trusted Core?") says
  plainly that Home derives this account's ARRR spending key and hands it to
  the trusted Core at the node shown, which keeps a synced copy; that the app
  receives only the address, balances and history; and that ending the
  session revokes the app's access, not the Core's copy. Rows: Account, Node,
  Shared with Core (spending key), Shared with app (address/balances/history),
  Not shared (seed). Gated on admin trust and an unlocked account; the route
  is pinned exactly like the bitcoiny consent (`nodeRoute` through prompt,
  grant key and post-approval recheck), and every guard re-reads state after
  the last await.
- **Entropy.** Hub-compatible, derived ONLY in the Electron main process,
  inside a callback for the lifetime of one request: for root seed `S`,
  account index `n` and wallet version `v`, `N = uint32BE(n)`, `M = N‖S‖N`,
  `A = S` (v = 1, the whole seed) else `SHA512(SHA512(M)‖M)[0:32]`,
  `R = reverse(copy(A))`, `entropy58 = Base58(SHA512(R‖SHA256(R‖"ARRR"))[0:32])`
  — Hub's `seed58`, before any secp256k1 normalization. Every intermediate
  buffer is zeroed in `finally`; the Base58 string cannot be zeroed, so its
  lifetime is one request body. It is never returned on an object, logged,
  journaled, persisted, or sent over IPC to the renderer or to an app, and
  every result and error is scrubbed of it in case a node ever echoed a body.
- **Reads.** `GET_USER_WALLET` → `walletaddress`; `GET_WALLET_BALANCE` →
  `walletbalance?verified=true` (`verified: false` for the total; a verified
  balance Core does not know is refused with `ARRR_VERIFIED_BALANCE_UNAVAILABLE`,
  never answered with the total); `GET_USER_WALLET_TRANSACTIONS` →
  `wallettransactions`, projected field by field into Core's
  `SimpleTransaction` shape (`txHash`, `timestamp`, `totalAmount`,
  `feeAmount`, `inputs`/`outputs` of `{ address, amount, addressInWallet }`,
  `memo`) — unknown keys are dropped and a `__proto__`/`constructor`/
  `prototype` key anywhere refuses the whole answer; `GET_ARRR_SYNC_STATUS` →
  `syncstatus?json=true`, validated into a frozen snapshot. Every answer is
  scrubbed of the entropy string (values AND property names) before any
  projection truncates node text. Redirects refused, responses bounded, 30 s
  timeout. One request in flight per Core route; a newer status poll from
  the same tab replaces one still queued (`ARRR_READ_SUPERSEDED`); at most
  four queued reads per app tab per route (`ARRR_READ_BACKLOG`, retryable);
  queued reads are cancelled on account/lock/node/tab lifecycle changes
  (`ARRR_READ_CANCELLED`). Core's 409 / `OPERATION_IN_PROGRESS` /
  `ARRR_WALLET_BUSY` becomes the retryable `ARRR_WALLET_BUSY` ("Your Core is
  busy with another ARRR wallet; try again shortly.") and never another
  account's data. Errors carry the PUBLIC route revision only, never the
  main-process credential digest.
- **Re-verification.** The orchestrator (`home-v2-arrr-custody-read.ts`,
  dependency-injected so the boundary is executed under test) re-checks the
  consent epoch, the live view context (same tab, same account), the unlock
  state, the live resource and a freshly re-resolved admin route (same route,
  same binding id, same credential revision) after the approval, again after
  the queue wait — before the seed is read — and again after the HTTP round
  trip before a result is delivered. A change at any of those points drops
  the result and refuses.
- **Send.** Not offered, and not offerable through this path: Core's ARRR
  `/send` would be Core spending the key, which is a different contract with
  its own journal and approval sequence. The capability row says
  `send: false`, `sendMode: "NONE"` unconditionally.
- **Android.** Deferred and advertised unavailable with a reason (the
  Android client derives foreign public data in the shared TypeScript layer,
  which is not a privileged boundary for a spending key). Android needs a
  native account-seed derivation with the identical vectors, an authenticated
  transport boundary, and the same consent and lifecycle checks before any of
  this is offered there.
...
pays the chain fee. The retained Home 1.x compatibility adapter additionally
accepts only the deployed top-level `{ base64, filename }` shape for a single
publish or an all-inline batch, after checking the 25 MiB aggregate decoded
byte budget before staging. It rejects mixed/token-plus-inline, path, nested,
and other encoding forms, then delegates to the existing bound consent,
signing, broadcast, result, and `finally` cleanup path. New apps should use
`STAGE_QDN_PUBLISH_SOURCE` and pass its token explicitly. `DELETE_QDN_RESOURCE` is implemented on `qdnRequest`
...
`qortalRequest`. The Hub-catalogue QDN-writes row above distinguishes the
narrow deployed `{ base64, filename }` compatibility form from retained Home
1.x examples that use broader inline/path fields; those broader forms stay
refused in favor of source tokens.
...
- `SEND_CHAT_MESSAGE` and the explicit public revision actions are the bounded
  ...
  (Qortal's retired general chat) is rejected with a specific error — Qortal
  General Chat goes through `SEND_QORTAL_GENERAL_CHAT`, which wraps the group-0
  CHAT the way Hub does (`electron/qortal-general-chat.ts`); on
