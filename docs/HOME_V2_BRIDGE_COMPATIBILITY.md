...
| Chat | Public group reads/writes, portable participation and administration, dual-chain avatars, and Qortium QDM1 plus Qortal legacy-v2 direct-message read/write/revision actions now use the same fine-grained contracts on desktop and Android. Direct crypto stays in Home, plaintext is not cached by the bridge, recipient/reference/account/route context is rechecked, and uncertain signed broadcasts remain non-retryable. | Chat must consume the new direct action family and complete its end-to-end route matrix. Private-group crypto, full resources, and the later distinct RCHAT family remain deferred to the portability roadmap. |
...
foreign wallets, private-group chat, Home/node settings writes, bookmarks,
