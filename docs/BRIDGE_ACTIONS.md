...
That gap is now closed on BOTH platforms by an independent field-by-field
verifier applied to the unstamped bytes and again to the nonce-stamped ones, so
nothing is signed that was not itself read back and confirmed: the type, the
...
`UNLOCK_SELECTED_ACCOUNT`, by contrast, is a Home-account
