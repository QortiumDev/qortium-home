...
Selecting an entry is an ephemeral renderer action. Home resolves that address
on both networks and shows one account with separately labelled Qortal and
Qortium presences. Selecting no account remains supported. This slice does not
write the active selection back to the wallet store.
