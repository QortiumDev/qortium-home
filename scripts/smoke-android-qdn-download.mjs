...
const fixtureName = process.env.QORTIUM_HOME_QDN_DOWNLOAD_FIXTURE_NAME ?? 'QortiumHomeTest';
const fixtureIdentifier = process.env.QORTIUM_HOME_QDN_DOWNLOAD_FILE_IDENTIFIER ?? 'home-file';
const fixtureAddress = `qdn://FILE/${fixtureName}/${fixtureIdentifier}`;
...
function log(message) {
  console.log(`[android-qdn-download-smoke] ${message}`);
}
...
async function launchEmulatorIfNeeded() {
  ...
  const logDir = path.join(os.tmpdir(), 'qortium-home-android-download-smoke');
...
async function assertFixturesReady() {
  const status = await getResourceStatus('FILE', fixtureName, fixtureIdentifier);

  if (status?.status !== 'READY') {
    fail(`FILE fixture is not READY at ${fixtureAddress}. Run npm run qdn:bootstrap-test-data first.`);
  }
}
...
async function configureSmokeNode(client) {
  ...
  if (!result?.ok) {
    fail(result?.message || `Unable to point Android download smoke test at ${androidNodeApiUrl}.`);
  }
}
...
async function clearDownloadedFiles(serial) {
  await adb(['-s', serial, 'shell', 'run-as', packageName, 'rm', '-rf', 'files/qdn-downloads'], {
    timeout: 10_000,
  }).catch(() => undefined);
}

async function getDownloadedFiles(serial) {
  const { stdout } = await adb(['-s', serial, 'shell', 'run-as', packageName, 'find', 'files/qdn-downloads', '-type', 'f'], {
    timeout: 10_000,
  });
...
async function waitForDownloadedFile(serial) {
  return waitUntil('Android QDN downloaded file', downloadTimeoutMs, async () => {
    const files = await getDownloadedFiles(serial).catch(() => []);

    return files.find((filePath) => filePath.includes('qortium-home-test-file')) ?? files[0] ?? null;
  });
}
...
async function waitForFileHandoff(serial) {
  return waitUntil('Android QDN file handoff', downloadTimeoutMs, async () => {
    const activity = await getTopActivity(serial);

    return activity && !activity.includes(packageName) ? activity : null;
  });
}

function getDownloadProbeExpression() {
  return `
    (async () => {
      ...
      const readyMessage = document.querySelector('.qdn-viewer__empty--ready .qdn-viewer__message')?.textContent?.trim() || '';
      ...
      const openButton = [...document.querySelectorAll('button')].find((button) =>
        (button.textContent || '').includes('Open') || (button.textContent || '').includes('Download')
      );

      if (errorMessage) {
        return { ready: false, errorMessage, loadingMessage, readyMessage, buttons, reason: 'viewer-error' };
      }

      if (!openButton) {
        return { ready: false, errorMessage, loadingMessage, readyMessage, buttons, reason: 'missing-open-button' };
      }

      if (!/ready to open|ready to download/i.test(readyMessage)) {
        return { ready: false, errorMessage, loadingMessage, readyMessage, buttons, reason: 'not-ready' };
      }
      ...
      return {
        ...
        ready: true,
        readyMessage,
        buttonText: openButton.textContent?.trim() || ''
      };
    })()
  `;
}

async function waitForDownloadReady(client) {
  let lastProbe = null;

  try {
    return await waitUntil('FILE download viewer', downloadTimeoutMs, async () => {
      const probe = await evaluate(client, getDownloadProbeExpression(), 'FILE download probe');
      lastProbe = probe;

      if (probe?.errorMessage) {
        fail(`FILE viewer showed an error: ${probe.errorMessage}`);
      }
    ...
    throw new Error(
      `${error instanceof Error ? error.message : String(error)} Last FILE probe: ${JSON.stringify(lastProbe)}.`,
    );
  }
}

async function clickDownloadButton(client) {
  const result = await evaluate(
    client,
    `
      (() => {
        const button = [...document.querySelectorAll('button')].find((item) =>
          (item.textContent || '').includes('Open') || (item.textContent || '').includes('Download')
        );

        if (!button) {
          return { ok: false, message: 'Open button was not found.' };
        }
    ...
    `,
    'click QDN download button',
  );

  if (!result?.ok) {
    fail(result?.message || 'Unable to click QDN download button.');
  }

  return result.text;
}

async function runDownloadAssertions(client, serial) {
  log(`Opening ${fixtureAddress}.`);
  await clearDownloadedFiles(serial);
  await navigateToAddress(client, fixtureAddress);
  const probe = await waitForDownloadReady(client);

  if (!/open/i.test(probe.buttonText)) {
    fail(`Expected Android QDN download button to say Open, found ${JSON.stringify(probe.buttonText)}.`);
  }

  log(`FILE viewer is ready with button "${probe.buttonText}".`);

  const clickedText = await clickDownloadButton(client);
  const downloadedFile = await waitForDownloadedFile(serial);
  const topActivity = await waitForFileHandoff(serial);

  log(`Clicked "${clickedText}", saved ${downloadedFile}, and reached ${topActivity}.`);
}
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await runDownloadAssertions(client, serial);
      } finally {
        client.close();
      }
    } finally {
    ...
    log('Android QDN download smoke test passed.');
  } finally {
    if (startedEmulator && process.env.QORTIUM_HOME_KEEP_ANDROID_EMULATOR !== '1') {
      ...
      await rm(path.join(os.tmpdir(), 'qortium-home-android-download-smoke'), {
        force: true,
        recursive: true,
      }).catch(() => undefined);
    }
